...
export default async function AdminRequestsPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900">Client requests — matching</h1>
      <p className="mt-1 text-slate-600">Review each request and assign the best assistant.</p>

      <div className="mt-6 space-y-4">
        {(requests ?? []).length === 0 && <p className="card p-6 text-sm text-slate-500">No client requests yet.</p>}
        {(requests ?? []).map((r) => (
          <div key={r.id} className="card p-5">
            ...
            <form action={updateClientRequest} className="mt-4 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-3">
