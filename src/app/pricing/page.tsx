  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-14">
      ...
      <section className="mt-12">
        <h2 className="text-xl font-semibold text-slate-900">Virtual mailboxes</h2>
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          {plans.map((p) => (
            <div key={p.id} className="card p-5">
              <h3 className="font-semibold text-slate-900">{p.name}</h3>
              <div className="mt-2 text-2xl font-bold">{money(p.price_cents)}<span className="text-sm font-normal text-slate-500">/{p.interval}</span></div>
              <Link href="/mailboxes" className="mt-3 inline-block text-sm font-semibold text-cyan-700 hover:underline">Choose plan →</Link>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mt-12">
        <h2 className="text-xl font-semibold text-slate-900">Assistants, marketing, websites & bundles</h2>
      ...
      <section className="mt-12">
        <h2 className="text-xl font-semibold text-slate-900">Office space</h2>
        <p className="mt-2 text-slate-600">
          Day offices from <strong>$45/day</strong>, desks from <strong>$25/day</strong>, meeting rooms billed per day.{" "}
          <Link href="/offices" className="font-semibold text-cyan-700 hover:underline">View & book offices →</Link>
        </p>
      </section>
    </div>
  );
