    ...
    <div className="mx-auto max-w-6xl px-4 py-14">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-3">
          {plans.map((p) => (
            <div key={p.id} className="card p-5">
              ...
              <Link href="/mailboxes" className="mt-3 inline-block text-sm font-semibold text-violet-300 hover:underline">Choose plan →</Link>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mt-12">
        ...
        <p className="mt-2 text-slate-400">
          ...
          <Link href="/offices" className="font-semibold text-violet-300 hover:underline">View & book offices →</Link>
        </p>
      </section>
    </div>
  );
