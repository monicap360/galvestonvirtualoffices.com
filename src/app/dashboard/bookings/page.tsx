  ...
  return (
    <div>
      ...
      {!bookings || bookings.length === 0 ? (
        <div className="card mt-6 p-8 text-center text-slate-400">No bookings yet.</div>
      ) : (
          ...
          {bookings.map((b) => {
            ...
            return (
              <div key={b.id} className="flex items-center justify-between gap-4 p-4">
                <div>
                  <p className="font-medium text-white">{office?.name ?? "Office"}</p>
                  <p className="text-sm text-slate-400">{dateLabel(b.start_date)} – {dateLabel(b.end_date)}</p>
                </div>
                <div className="text-right">
                  <p className="font-semibold text-white">{money(b.total_cents)}</p>
