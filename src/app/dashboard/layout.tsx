...
const nav = [
  ...
  { href: "/dashboard/start", label: "Get Started" },
  { href: "/dashboard/agents", label: "My AI Agents" },
    ...
    <div className="mx-auto max-w-6xl gap-8 px-4 py-8 lg:flex">
      <aside className="lg:w-56 lg:shrink-0">
        <div className="card p-4">
          ...
          <p className="truncate font-semibold text-white">{profile.full_name || profile.email}</p>
          <nav className="mt-4 flex flex-row flex-wrap gap-1 lg:flex-col">
            ...
            {isStaff && (
              <Link href="/admin" className="rounded-lg px-3 py-2 text-sm font-semibold text-violet-300 hover:bg-violet-400/10">
                Admin →
              </Link>
            )}
          </nav>
