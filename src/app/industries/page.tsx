  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-16">
      <header className="text-center">
        <p className="text-sm font-semibold uppercase tracking-[0.25em] text-cyan-300/80">Industries</p>
      ...
      <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {INDUSTRIES.map((i) => (
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-wide text-cyan-300/80">{i.eyebrow}</p>
            <h2 className="mt-2 text-xl font-semibold text-white group-hover:text-cyan-300">{i.name}</h2>
            <p className="mt-2 text-sm text-slate-400">{i.headline}</p>
            <span className="mt-4 inline-block text-sm font-semibold text-cyan-300">See the solution →</span>
          </Link>
        ))}
      </div>
    </div>
