...
const agentTry = fs.readFileSync("src/app/ai-studio/agent-try.tsx", "utf8");

assert.ok(
  page.includes("Choose a prebuilt") && page.includes("AI employee."),
  "marketplace hero must sell the prebuilt-agent concept",
);
assert.ok(card.includes("<AgentTry") && agentTry.includes("Try a demo"), "cards must keep the live demo action");
